...
def getListParents(id_last_child):
    def repeat(repeat_count, all_projects, id_last_child):
        if repeat_count < MAX_CHIAMATE:
            ...
            all_parents_project = list(
                Repository.objects.get(gitlab_pid=id_last_child).parents.all()
            )
            # scorro per ogni parent la funzione ricorsiva
            for parent in all_parents_project:
                # aggiungo il parent alla lista di tutti i projects
                try:
                    all_projects.remove(parent)
                except Exception:
                    pass
                all_projects.append(parent)
                all_projects, repeat_count = repeat(
                    repeat_count, all_projects, parent.gitlab_pid
                )
    ...
    # per il controllo "cane che si morde la coda"-->
    #  se un child è il padre di un padre si crea un anello infinito-->
    # metto un contatore per ogni volta che viene richiamata la funzione
...
def convert_data(arg):
    return (
        datetime.datetime.fromisoformat(arg.commit["committed_date"]).timestamp() * 1000
    )
...
def tagControl(
    last_tag_date,
    defaultBranch_date,
    id_commit_defaultBranch,
    id_commit_last_tag,
    merge_request,
    last_tag_parent_date,
    is_protected,
    ...
    if last_pipeline_status == "success":
        if (
            id_commit_defaultBranch == id_commit_last_tag
            and merge_request is False
            and last_tag_parent_date <= last_tag_date
            and last_pipeline_status == "success"
        ):
            return "green", ""
        if defaultBranch_date > last_tag_date:
            return "yellow", "default branch più giovane dell'utlimo tag"
        if merge_request is True:
...
def protected_tags_in_branch(gl, branches, id_project):
    for branch in branches:
        ...
        if len(tags) > 0:
            ...
            if branch.default is not True and last_tag.protected is True:
                return True
    return False
...
def setStatus(all_projects, project, status):
    ...
    try:
        ...
        return True
    if status_project != "red" or status_project is None:
        if status == "red" and (
            status_project == "green"
            or status_project == "yellow"
            or status_project == "blue"
        ):
            return True
...
def setGray(all_projects, projects_red):
    ...
    for project_red in projects_red:
        ...
        for child in children:
            try:
                # trovo il project corrispondente a child in all_projects
                # cambio lo stato
...
def getDependecyStatus(gl, id_last_child):
    ...
    all_projects, count = getListParents(
        id_last_child=id_last_child
    )  # count viene ritornato per i test    merge_request = False
    if count < 50:
        all_projects_exist, all_projects = exist_repositories_on_gitlab(
            all_projects, gl
        )
        if all_projects_exist is True:
            # lista contenente tutti i projects rossi-->
            # calcolerò i descendants di ogni project sarà grigio
            projects_red = list()
            # controllo lo stato1
            for project in all_projects:
                # default branch
                ...
                project.save()
                branches = (
                    repository.branches.list()
                )  # lista contenente il tag corrente
                # controllo che non ci siano dei tag protetti
                # in branch che non siano di default--> True--> rosso
                is_protected = protected_tags_in_branch(
                    gl, branches, project.gitlab_pid
                )
                for branch in branches:
                ...
                if len(tags) > 0:
                    last_tag = tags[
                        len(tags) - 1
                    ]  # corrisponde all'ultimo tag della lista
                    # #trovo la data del default branch
                    defaultBranch_date = datetime.datetime.fromisoformat(
                        defaultBranch.commit["committed_date"]
                    ).timestamp()
                    # #trovo la data del last tag
                    last_tag_date = datetime.datetime.fromisoformat(
                    ...
                    for pipeline in pipelines:
                        ...
                        if is_tag_pipeline is True:
                            last_pipeline_tag = pipeline
                            last_pipeline_status = (
                                last_pipeline_tag.status
                            )  # salvato in ordine dalla più nuova alla più vecchia
                            break
                    ...
                    for parent in all_parents_project:
                        ...
                        if len(tags_parent) == 0:
                            ...
                            projects_red.append(
                                project
                            )  # bisogna salvarsi il project rosso
                            # per poter trovare i suoi child
                            # da mettere grigi alla fine del controllo
                            #  di tutti gli altri project
                            # print(f"{project.project_name}  {project.status}")
                            continue

                        last_tag_parent = tags_parent[len(tags_parent) - 1]
                        last_tag_parent_date = datetime.datetime.fromisoformat(
                            last_tag_parent.commit["committed_date"]
                        ).timestamp()

                        # controllo lo stato
                        # controllo che i project con uno stato più grave
                        # non vengano cambiati
                        ...
                        if setStatus(all_projects, project, status) is True:
                        ...
                        if (
                            project.status == "red"
                        ):  # rosso è il projct corrente--> i figli diventano grigi
                            projects_red.append(
                                project
                            )  # bisogna salvarsi il project rosso
                            # per poter trovare i suoi child
                    ...
                    if (
                        len(all_parents_project) == 0
                    ):  # and setStatus(all_projects, project) == True:
                        # controllo che i project con uno stato più grave
                        # non vengano cambiati
    ...
    return all_projects  # la lista contiene tutti i projects con il colore dello stato


#                          alla voce status di ogni project
